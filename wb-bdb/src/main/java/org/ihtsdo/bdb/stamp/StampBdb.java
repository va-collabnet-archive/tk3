        ...
        synchronized (uncomittedStatusPathEntries) {
            ...
            try {
                ...
                for (int i = min; i < sequence.get(); i++) {
                    ...
                    stampToIntMap.put(getStatusNid(i), Long.MIN_VALUE, getAuthorNid(i), getModuleNid(i), getPathNid(i), i);
                }
        ...
        synchronized (uncomittedStatusPathEntries) {
            ...
            try {
                for (int sapNid : uncomittedStatusPathEntries.values()) {
                    ...
                    stampToIntMap.put(getStatusNid(sapNid), time, getAuthorNid(sapNid), getModuleNid(sapNid), getPathNid(sapNid),  sapNid);
        ...
        try {
            ...
            for (int i = 0; i < readOnlySize; i++) {
                if (readOnlyArray.commitTimes[i] != 0) {
                    stampToIntMap.put(readOnlyArray.statusNids[i], readOnlyArray.commitTimes[i], readOnlyArray.authorNids[i],
            ...
            for (int i = 0; i < mutableSize; i++) {
                ...
                if (mutableArray.commitTimes[i] != 0) {
                    stampToIntMap.put(mutableArray.statusNids[i], mutableArray.commitTimes[i], mutableArray.authorNids[i],
