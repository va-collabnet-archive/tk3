...
public class EnumPropertyKeyHelper {
     ...
     * <p>
     * Specifically, this method will take the package name and if the 
     * class is an inner class, will replace the dollar sign
     * with a .  For example:
     * <ul>
    ...
    public static String getFullKeyString(Class<?> keyClass) {
        return keyClass.getName().replace('$', '.');
    }
   ...
   /**
    ...
    private static String shortenClassNameToUUID(Class<?> keyClass) {
        String prefix = keyClass.getPackage().getName();
